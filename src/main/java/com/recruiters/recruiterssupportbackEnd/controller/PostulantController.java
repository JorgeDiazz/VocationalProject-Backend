...
public class PostulantController {
    ...
    @PostMapping("/")
    public ResponseEntity<UserEntity> createPostulant(@RequestBody CreatePostulantRequest newPostulant, @RequestHeader(value = "Authorization") String token) throws Throwable {
    ...
    @PutMapping("/")
    public ResponseEntity<UserEntity> updatePostulant(@RequestBody CreatePostulantRequest newPostulant, @RequestHeader(value = "Authorization") String token) throws Throwable {
        if (Integer.parseInt(ResponseUtils.Validation(token).get(0)) != 1 /*&& ResponseUtils.Validation(token).get(1)== "COMPANY"*/) {//1 para no se vencio   
            throw new UnauthorizedException("Validation Problem");
        } else {
