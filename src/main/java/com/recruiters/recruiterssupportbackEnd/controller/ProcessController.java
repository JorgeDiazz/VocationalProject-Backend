...
public class ProcessController {
    ...
    @GetMapping("/{nit}")
    public List<Processs> getProcessesByJob(@PathVariable int nit, @RequestHeader(value = "Authorization") String token) throws UnauthorizedException {
    ...
    public ResponseEntity<Processs> createProcess(@RequestBody CreateRequestProcess createRequestProcess, @RequestHeader(value = "Authorization") String token) throws Throwable {
        if (Integer.parseInt(ResponseUtils.Validation(token).get(0)) != 1 /*&& ResponseUtils.Validation(token).get(1)== "COMPANY"*/) {//1 para no se vencio   
            throw new UnauthorizedException("Validation Problem");
        } else {
