...
public class AreaController {
    ...
    @GetMapping("/{nit}")
    public ResponseEntity<List<Area>> getAllAreaByNit(@PathVariable String nit, @RequestHeader(value = "Authorization") String token) throws UnauthorizedException {
    ...
    public ResponseEntity<Area> createArea(@RequestBody CreateRequestArea createRequestArea, @RequestHeader(value = "Authorization") String token) throws Throwable {
        if (Integer.parseInt(ResponseUtils.Validation(token).get(0)) != 1 /*&& ResponseUtils.Validation(token).get(1)== "COMPANY"*/) {//1 para no se vencio   
            throw new UnauthorizedException("Validation Problem");
        } else {
